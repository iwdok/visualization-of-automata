...
import dfma.base.exceptions as exceptions
import dfma.pda.exceptions as pda_exceptions
import dfma.pda.pda as pda
from dfma.pda.configuration import PDAConfiguration
from dfma.pda.stack import PDAStack
...
class DPDA(pda.PDA):
    ...
    def __repr__(self):
        full = ""
        for start_state, paths in self.transitions.items():
            for second_state, paths1 in paths.items():
                for third_state, paths2 in paths1.items():
                    full = full + "({0} {1} {2} {3}) \n".format(start_state, second_state, third_state, paths2[0])
        return full
    ...
    def _get_next_configuration(self, old_config):
        ...
        if len(transitions) == 0:
            raise exceptions.RejectionException(
                'Автомат вошел в конфигурацию, для которой нет '
                'перехода ({}, {}, {})'.format(
                    old_config.state,
                    old_config.remaining_input[0],
                    old_config.stack.top()
    ...
    def get_table(self):
        ...
        for start_state, paths in self.transitions.items():
            for second_state, paths1 in paths.items():
                for third_state, paths2 in paths1.items():
                    full.append([start_state, second_state, third_state, paths2[1], paths2[0]])
        return full
